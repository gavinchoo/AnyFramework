...
package com.android.zcomponent.views.swipebacklayout.lib;
...
public class SwipeBackLayout extends FrameLayout
{
	...
	public SwipeBackLayout(Context context, AttributeSet attrs, int defStyle)
	{
		...
		TypedArray a =
				context.obtainStyledAttributes(attrs, R.styleable.SwipeBackLayout, defStyle,
	...
	public void setSlidViewPosition(int finalLeft, int finalTop)
	{
	...
	{
		mDialog = dialog;
		TypedArray a = dialog.getWindow().getWindowStyle();
		int background = a.getResourceId(0, 0);
		a.recycle();
	...
	{
		mFragment = fragment;
		TypedArray a = fragment.getActivity().getWindow().getWindowStyle();
		int background = a.getResourceId(0, 0);
		a.recycle();
	...
	private class ViewDragCallback extends ViewDragHelper.Callback
	{
		...
		public void onViewPositionChanged(View changedView, int left, int top, int dx, int dy)
		{
			...
			if (mListeners != null && !mListeners.isEmpty())
			{
				for (SwipeListener listener : mListeners)
				{
					listener.onScrollPercent(mScrimOpacity);
				}
			}
			...
			if (mScrollPercent >= 1)
			{
				...
				{
					LogEx.d(VIEW_LOG_TAG, "mDialog.dismiss");
					mDialog.dismiss();
					handler.sendEmptyMessageDelayed(0, 200);
				}

				if (null != mFragment && !mFragment.isDetached())
				{
		...
		public void onViewReleased(View releasedChild, float xvel, float yvel)
		{
			...
			{
				left =
						xvel > 0 || xvel == 0 && mScrollPercent > mScrollThreshold ? childWidth
			...
			{
				top =
						yvel < 0 || yvel == 0 && mScrollPercent > mScrollThreshold ? -(childHeight
